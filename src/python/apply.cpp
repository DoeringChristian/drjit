...
uint64_t TraverseCallback::traverse_rw(uint64_t) { return 0; }
void TraverseCallback::traverse_unknown(nb::handle) { }

/// Invoke the given callback on leaf elements of the pytree 'h'
void traverse(const char *op, TraverseCallback &tc, nb::handle h, bool traverse_rw) {
    nb::handle tp = h.type();
        ...
        if (is_drjit_type(tp)) {
            ...
            if (s.is_tensor) {
                ...
                for (Py_ssize_t i = 0; i < len; ++i)
                    traverse(op, tc, nb::steal(s.item(h.ptr(), i)), traverse_rw);
            } else  {
                tc(h);
            }
        } else if (tp.is(&PyTuple_Type)) {
            for (nb::handle h2 : nb::borrow<nb::tuple>(h))
                traverse(op, tc, h2, traverse_rw);
            ...
            if (nb::dict ds = get_drjit_struct(tp); ds.is_valid()) {
                for (auto [k, v] : ds)
                    traverse(op, tc, nb::getattr(h, k), traverse_rw);
            } else if (nb::object df = get_dataclass_fields(tp); df.is_valid()) {
                for (nb::handle field : df) {
                    nb::object k = field.attr(DR_STR(name));
                    traverse(op, tc, nb::getattr(h, k), traverse_rw);
                }
            } else if (nb::object cb = get_traverse_cb_ro(tp); cb.is_valid() && !traverse_rw) {
                cb(h, nb::cpp_function([&](uint64_t index) { tc(index); }));
            } else if (nb::object cb = get_traverse_cb_rw(tp); cb.is_valid() && traverse_rw) {
                cb(h, nb::cpp_function([&](uint64_t index) { return tc.traverse_rw(index); }));
            } else {
